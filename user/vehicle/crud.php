...
include ("config.php");
  ...
  if(isset($_POST['tambah'])){
    ...
    $app_sql = "SELECT booking_id FROM booking_vehicle WHERE coach_id = '$coach' AND booking_status = '$status'";
  ...
  if($app == 0){
            ...
            $sql = "INSERT INTO booking_vehicle (vehicle_id, coach_id, booking_start, booking_end, booking_status)
    ...
    <main class="main">
          ...
          <?php } ?>
            <header class="section__header">
              <h2 class="section__title">Senarai Tempahan Pengangkutan</h2>
              <div class="section__control">
                ...
                >
                  <img src="styleAdm/ico/add-car.svg" width="32">
                </button>
              </div>
            </header>
            <ul class="project">
                <div  class="project__item_1" style="display: none;">
                <h4>Tempah Pengangkutan</h4>
                <form class="gform_wrapper" onsubmit="successBox()" method="POST" action="">
                    <span>
                    <select class="input--style-3" type="text" name="vehicle_id" style="color: rgba(255, 255, 255, 0.7) !important;"required>
                        <option value="" style="color: rgba(255, 255, 255, 0.7) !important;">Pilih Pengangkutan</option>
                          <?php 
                                $facility_q = "SELECT * FROM vehicle";
                                $result_p = mysqli_query($link, $facility_q);
                                while($row=mysqli_fetch_assoc($result_p)){
                          ?>
                          <option value="<?= $row['vehicle_id']; ?>"><?= $row['vehicle_noplate']; ?></option>
                          <?php } ?>
                        </select>
                    </span>
                    <span>
            ...
            ?>
              ...
              <table class="data">
                ...
                $sql = "SELECT * FROM booking_vehicle b JOIN vehicle s ON s.vehicle_id = b.vehicle_id LIMIT $offset, $no_of_records_per_page";
